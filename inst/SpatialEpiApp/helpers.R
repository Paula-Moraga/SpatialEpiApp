...
#' @param idinmap
#' @param temporalunit character that can be "year", "month", "day"
#' @param usedarealcovs names of the area level covariates.
...
fnCheckData<-function(datos, usedcovs, map, idinmap, temporalunit, usedarealcovs){
...
################################
# INI Check areal covariates
################################
# if there areal level covariates, check there are all numeric and there are no missing values
if(length(usedarealcovs)>0){

# All columns numeric? TRUE or FALSE
# All columns give TRUE when check is.numeric?
allcolumnsnumeric <- sum(sapply(data.frame(datos[, usedarealcovs]), is.numeric)) == length(usedarealcovs)
if(!allcolumnsnumeric){
  return("Incorrect file input. Areal covariates are not numeric values.")
}
# There are some values that are NAs?
thereareNAs <- sum(sapply(datos[, usedarealcovs], is.na)) > 0
if(thereareNAs){
  return("Incorrect file input. There are missing values in areal covariates.")
}
}


################################
# END Check areal covariates
################################
...
#'
#'
fnConstructData<-function(datos, numstrata, vecid, vectime, temporalunit, timeformattedIfSpatial, usedarealcovs){
...
# areal level covariates
if(length(usedarealcovs)>0){
for(i in 1:length(usedarealcovs)){
    datos2[, usedarealcovs[i]]<- aggregate(x=datos[, usedarealcovs[i]], by=list(time = datos$time, id = datos$id), FUN="mean")[,"x"]
  }}
...
#'
fnEstimateRisk <- function(datos, map, usedarealcovs) {
  ...
  formulacovariates<-""
  if(length(usedarealcovs)>0){
    formulacovariates <- paste(paste(usedarealcovs, collapse = "+"),"+")
  }
  #Formula spatial
  #formula = Observed ~ f(ID.area, model="besag", graph=g, adjust.for.con.comp=TRUE) + f(ID.area1, model="iid")
  formula <- as.formula(paste(" Observed ~ ",formulacovariates,"f(ID.area, model='besag', graph=g, adjust.for.con.comp=TRUE) + f(ID.area1, model='iid')"))
  ...
  # Bernardinelli
   ...
   formula <- as.formula(paste(" Observed ~ ",formulacovariates,"f(ID.area, model='bym', graph=g, adjust.for.con.comp=TRUE) + f(ID.area1, ID.time, model='iid',constr=TRUE) + ID.time"))
  }
  ...
  datos$RiskTimesExpectedLLCI <- datos$LowerLimitCI * datos$Expected
  datos$RiskTimesExpectedULCI <- datos$UpperLimitCI * datos$Expected
